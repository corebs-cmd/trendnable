-- Migration 016: add mercari_url and popnbeats_url to skus
-- These are optional admin-set URLs shown in the app's "Where to buy" section.
-- A marketplace is only shown in the app when its URL is explicitly set.

alter table skus add column if not exists mercari_url   text;
alter table skus add column if not exists popnbeats_url text;

-- Rebuild view to expose the two new columns
